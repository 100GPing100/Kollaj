(function () {
	...
	var defaults = {
		evalPath: isNode ? __dirname + '/eval.js' : "https://kollaj.net/util/eval.js",
			...
			try {
				if (this.requiredScripts.length !== 0) {
					if (this.options.evalPath !== null) {
						wrk = new Worker('js/util/eval.js');
						wrk.postMessage(src);
					} else {
				...
				if (this.options.evalPath !== null) { // blob/url unsupported, cross-origin error
					console.log([src].toString()+[this.options.evalPath].toString());
					wrk = new Worker(URL.createObjectURL(new Blob(["("+[src].toString()+[this.options.evalPath].toString()+")()"], {type: 'text/javascript'})));
