...
(function(root){
	...
	function SvgNest(){
		...
		this.launchWorkers = function(tree, binPolygon, config, progressCallback, displayCallback){
			...
			p.map(function(pair){
//				console.log (pair.key.Arotation);
		...
		this.applyPlacement = function(placement){
			...
			for(i=0; i<placement.length; i++){
				...
				for(j=0; j<placement[i].length; j++){
					...
					partgroup.setAttribute("id", "nested" + (parseInt(part.source)-1));
					partgroup.appendChild(clone[part.source]);
					//FIXME:: ok so here I have a workaround.
					//instead of passing the ID to svgNest, what we do is
					// workaround by use thing fact that it automatically counts the nodes in the svgDoc.
					//I think, we should fix this in the future.
					//Kristian

					if(part.children && part.children.length > 0){

						/*setId = part.children[0].id;
						alert(part.children[0])
						part.setAttribute('id', setId);

						var c = clone[part.children[0].source];
						console.log(c.getAttribute('class'));
						part.setAttribute('class',part.children[0].getAttribute('class'));
    ...
    this.config = { populationSize: 30, mutationRate: 25, rotations: 4 };
    ...
    /*
		...
		*/
    // so I changed this so that we dont need to bother with rotation numbers,
    // instead of (what I originally thought -45 -- +45 ) it's now a -30 to +30 range, so we have better results
