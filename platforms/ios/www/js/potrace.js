...
(function (potrace) {
    ...
    var PathList = (function (_super) {
        ...
        //scale 1 = 100%
        //
        //elementID = id of the path created by potrace
        //fillRGBA = rgba "0,0,0,0.8" value of fill color

        //transform = [transformX, transformY, transformRotate]
        //stroke = strokeWidth
        PathList.prototype.toSVG = function (scale, elementID, fillRGBA, transform, stroke) {
            ...
            for (var i = 0, len = this.length; i < len; ++i) {
                ...
                for (var i_6 = 0, j = 0; j < n; ++i_6, j += 3) {
                    ...
                    else if (curve.tag[i_6] === 0 /* Corner */) {
                        svg.push('L ' + (c[j + 1].x * scale).toFixed(3) + ' ' +
                            (c[j + 1].y * scale).toFixed(3) + ' ');
                        svg.push((c[j + 2].x * scale).toFixed(3) + ' ' +
                            (c[j + 2].y * scale).toFixed(3) + ' ');
            ...
            if (stroke) {
                ...
                svg.push(' stroke="none" ');
            }

            if(transform)
            {
              svg.push(' transform="translate('+transform[0]+' '+transform[1]+') rotate('+transform[2]+')" ')
            }

            if(fillRGBA)
            {
              svg.push(' fill="rgba('+fillRGBA+')" ');
            }
            if(elementID)
            {
              svg.push(' class="'+elementID+'" ');
            }
    ...
    function fromImage(src, opt) {
        opt = opt || {};
        return PathList.fromBitmap(Bitmap.createFromImage(src), 'turnPolicy' in opt ? opt.turnPolicy : 4 /* Minority */, 'turdSize' in opt ? opt.turdSize : 2, 'alphaMax' in opt ? opt.alphaMax : 1, 'optCurve' in opt ? opt.optCurve : false, 'optTolerance' in opt ? opt.optTolerance : 0.0);
    }
    potrace.fromImage = fromImage;
    function fromFunction(f, width, height, opt) {
        opt = opt || {};
        return PathList.fromFunction(f, width, height, 'turnPolicy' in opt ? opt.turnPolicy : 4 /* Minority */, 'turdSize' in opt ? opt.turdSize : 2, 'alphaMax' in opt ? opt.alphaMax : 1, 'optCurve' in opt ? opt.optCurve : false, 'optTolerance' in opt ? opt.optTolerance : 0.0);
    }
